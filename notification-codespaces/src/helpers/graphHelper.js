...
class GraphHelper {
    ...
    GetAccessToken() {
        ...
        const data = qs.stringify({
            ...
            'client_secret': process.env.BOT_PASSWORD//process.env.MicrosoftAppPassword
        });

        return new Promise(async (resolve) => {
            const config = {
                method: 'post',
                url: 'https://login.microsoftonline.com/' + "4628ca42-554a-46cb-b1cc-79c95e7c9352" + '/oauth2/v2.0/token',
                headers: {
    ...
    async GetApplicationInternalId() {
        try
        {
            var apps =  await this.graphClient.api(`/appCatalogs/teamsApps`).filter(`externalId eq '${process.env.TEAMS_APP_ID}'`).get();
